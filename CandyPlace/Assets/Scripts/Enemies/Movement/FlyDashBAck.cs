...
public class FlyDashBAck : EnemyCombat
{
    ...
    void Dash()
    {
        ...
        while (currentPosition.x - transform.position.x > .1 && currentPosition.y - transform.position.y > .1 && isMoving)
        {
            Vector3 transfers = currentPosition - transform.position;
            transfers = Vector3.Normalize(transfer);
            GetComponent<Rigidbody2D>().velocity = transfer * speed * 5;
        }
        stun = true;
        stunTimer = 0;
